...
int sendAccelEvent(int paramNum,int *pValue)
{
	...
	for(i=0; i < paramNum; i++){
	    ...
	    event.value = *pValue++;
	    ALOGE("SensorJni:sendAccelEvent write sensor event to %s, type=%d code=%d , value=%d",ACCEL_SENSOR_DATA_DEVICE,event.type,event.code,event.value);
...
{
	if(-1 != s_accel_fd){
		ALOGE("SensorJni:sensor device has already opened !!!\n");
		return -1;
	}
...
{
	int ret = -1;
	ALOGE("SensorJni :Java_com_external_cameraService_helper_SensorJniHelper_sendSensorEventRemote: type=%d, x=%d,y=%d,z=%d",sensorType,x,y,z);
	switch(sensorType)
	{
	...
	*/
		default:
			ALOGE("SensorJni: ERROR: unsupport sensor type:  %d \n", sensorType);
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) 
{
	...
	ret = openSensorDevice();
	if(ret < 0 )
	{
		ALOGE("SensorJni: failed open device\n");
		return -1;
	}
